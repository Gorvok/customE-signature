...
export default function CollapsibleSection({ id, title, subtitle, icon, filled, open, onToggle, children }: Props) {
  ...
  return (
    ...
    >
      {/* The disclosure button is the section's heading, so screen-reader users can jump between sections. */}
      <h2>
        <button
          type="button"
          onClick={onToggle}
        ...
        >
          ...
          <span className="flex-1 min-w-0">
            ...
            {subtitle && <span className="block text-xs font-normal text-hint truncate">{subtitle}</span>}
          </span>
          <svg
            className={`flex-shrink-0 text-muted transition-transform duration-300 ${open ? 'rotate-180' : ''}`}
            width="18"
            height="18"
            viewBox="0 0 24 24"
            fill="none"
      ...
      {/* inert: a collapsed section's inputs leave the tab order and the accessibility tree, not just the screen. */}
      <div
        id={contentId}
        inert={!open}
