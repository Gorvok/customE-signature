...
export const elegant: SignatureTemplate = {
  ...
  render: (data: SignatureData, options = {}) => {
    ...
    return `<table cellpadding="0" cellspacing="0" border="0" style="font-family: Georgia, 'Times New Roman', ${font}, serif; text-align: center;">
  <tr>
    <td align="center">
      <table cellpadding="0" cellspacing="0" border="0">
        ${logoHtml}
        <tr>
          <td align="center" style="font-size: 20px; color: ${textColor}; font-family: Georgia, 'Times New Roman', serif; letter-spacing: 2px;">${esc(data.fullName) || 'Your Name'}</td>
        </tr>
        ${data.jobTitle || data.company ? `<tr><td align="center" style="font-size: 11px; color: ${accent}; font-family: ${font}, sans-serif; padding-top: 4px; text-transform: uppercase; letter-spacing: 3px;">${[data.jobTitle, data.company].filter(Boolean).map(esc).join('  &mdash;  ')}</td></tr>` : ''}
        <tr>
